"""Package the trained **crustle** policy into a self-contained submission bundle.

    python archetypes/crustle/make_submission.py

Produces ``submission_crustle/`` plus ``submission_crustle.zip`` containing:

    main.py                                  agent() entry point (inference)
    deck.csv                                 60 card ids
    bc_policy.pt                             trained weights
    cg/                                      game engine (incl. native libs)
    archetypes/crustle/policy_network/       feature pipeline + network
...
The deck defaults to ``archetypes/crustle/crustle_deck.csv`` — the decklist
reconstructed from the ``flg`` replays this policy was cloned from — not the
repo-root ``deck.csv`` or ``crustle_deck.csv``, which are different lists the
network has never seen played. The weights default to
``checkpoints/bc_policy_crustle.pt``, the best-epoch checkpoint bc_train.py
saves (its ``.final``/``.resume`` siblings are the last epoch and the
resume state, neither of which is what you want to submit).

Verify the result before submitting:

    python archetypes/crustle/make_submission.py --verify
...
# The repo root, two levels up from archetypes/crustle/ — everything this
# script copies (cg/, submission_main.py, checkpoints/) lives there, not
# beside this file.
_ROOT = Path(__file__).resolve().parents[2]
_ARCHETYPE = Path(__file__).resolve().parent
...
_POLICY_DIR = Path("archetypes/crustle/policy_network")
...
def build(deck: Path, checkpoint: Path, out_dir: Path, zip_path: Path | None) -> Path:
    ...
    if missing:
        raise SystemExit(
            ...
            + "\n(build the deck with archetypes/crustle/build_crustle_deck.py, "
              "train weights with archetypes/crustle/policy_network/bc_train.py)"
        )
...
def main() -> None:
    ...
    parser.add_argument("--deck", default=str(_ARCHETYPE / "crustle_deck.csv"))
    parser.add_argument("--checkpoint", default=str(_ROOT / "checkpoints/bc_policy_crustle.pt"))
    # Distinct from the repo-root ``submission/``, which the alakazam
    # packager owns: building one archetype must not silently overwrite
    # another's bundle, since both produce a file called bc_policy.pt.
    parser.add_argument("--out", default=str(_ROOT / "submission_crustle"))
