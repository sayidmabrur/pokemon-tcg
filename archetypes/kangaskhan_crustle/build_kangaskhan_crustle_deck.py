...
trained on one player's decisions, so it has only ever seen that player's
own decklist. Handing the clone a different deck at evaluation time asks it
to pilot cards it never watched anyone play — cards whose embeddings may
still be at their random initialisation — which measures the deck mismatch
rather than the policy. Evaluating the clone on the deck it was cloned from
is the apples-to-apples comparison that isolates "did the model learn
anything" from "is the deck wrong".

This archetype clones the ``Where is my orbit`` Mega Kangaskhan ex /
Dwebble-Crustle deck (Kaggle submission 54959248, 611 episodes). Note it
shares most of its core with ``crustle-agent-rule-based/deck.csv`` — the
same Kangaskhan/Crustle line and the same Mist/Spiky/Grow Grass energy
package — so ``duel_inference.py``'s default opponent is close to a mirror
match here, which makes it a clean policy-vs-heuristics comparison on
near-identical decks rather than a cross-archetype one.
...
def visible_cards(player: dict) -> collections.Counter:
    ...
    for pokemon in (player["active"] or []) + (player["bench"] or []):
        ...
        # especially: an Alakazam on the board means its Kadabra and Abra
...
def main() -> None:
    ...
    parser.add_argument("--parquet", default=str(_REPO_ROOT / "data/policy_decisions_kangaskhan_crustle.parquet"))
    parser.add_argument("--player-name", default="Where is my orbit")
    parser.add_argument("--card-data", default=str(_REPO_ROOT / "EN_Card_Data.csv"))
    parser.add_argument("--out", default=str(_REPO_ROOT / "kangaskhan_crustle_deck.csv"))
