...
Run it::
    ...
    python archetypes/lucario/duel_inference.py --episodes 100
...
The BC policy defaults to ``lucario_deck.csv``, the list reconstructed from
...
# This file lives in archetypes/lucario/ (not the repo root), so the repo
...
def duel(
    ...
    the Lucario list makes it play badly for reasons that have nothing to do
...
def main() -> None:
    ...
    parser.add_argument(
        "--deck", default=str(_REPO_ROOT / "lucario_deck.csv"),
