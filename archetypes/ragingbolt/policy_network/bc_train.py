...
# duel_inference.py lives in archetypes/ragingbolt/ (one level up from this
...
def train(
    parquet_path: str = "data/policy_decisions_ragingbolt.parquet",
    player_name: str = "James Cox & Henry Chao",
    epochs: int = 1,
    lr: float = 1e-3,
    grad_clip: float = 1.0,
    batch_size: int = 64,
    ...
    if duel_episodes > 0:
        duel_deck_path = Path(duel_deck or duel_inference._REPO_ROOT / "ragingbolt_deck.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--parquet", default="data/policy_decisions_ragingbolt.parquet")
    ...
    parser.add_argument(
        ...
        help="deck for the policy in the end-of-epoch duel; defaults to ragingbolt_deck.csv",
    )
    parser.add_argument(
