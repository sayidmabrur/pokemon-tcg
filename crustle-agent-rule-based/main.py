...
class Ctx:
    ...
    def maybe_in_deck(self, cid) -> bool:
        return DECK_COUNTS.get(cid, 0) - self.visible_count(cid) > 0
    ...
    def mist_priority(self) -> bool:
        ...
        placed damage counters bypass our walls and only Mist Energy stops
        them, so the Active Spot must carry a Mist Energy."""
        for p in self.opp_in_play():
            if is_counter_placer(p.id):
                return True
        return False
    ...
    def opp_gust_threat(self) -> bool:
        """Opponent has shown they can pull our bench into the Active Spot
        (Boss's Orders in their discard, or a Heave-Ho style Pokemon seen
        in play or in their discard)."""
        for c in self.op.discard:
            if c.id == BOSS or has_gust_ability(c.id):
                return True
        for p in self.opp_in_play():
            if has_gust_ability(p.id):
                return True
        return False
...
def fetch_score(ctx: Ctx, cid: int) -> float:
    ...
    if cid == ROCK_F:
        ...
        return 22
    if cid == ULTRABALL:
        return 65 if f["latias_need"] else 24
    if cid == ICECREAM:
        a = ctx.my_active()
...
def bench_score(ctx: Ctx, cid: int) -> float:
    ...
    if cid == LATIAS:
        if LATIAS in in_play:
            return -50
        if ctx.opp_gust_threat():
            return -10  # do not hand gust decks a 2-prize bench target
        return 85
    if cid == KANGA:
...
def item_score(ctx: Ctx, cid: int) -> float:
    ...
    if cid == TRIMMER:
        ...
        return 0
    if cid == POFFIN:
        if (ctx.search_ok() and len(ctx.line_in_play()) < 3
                and ctx.maybe_in_deck(DWEBBLE)
                and len(ctx.my.bench) < ctx.my.benchMax):
            return 700
        return 0
    if cid == ULTRABALL:
    ...
    for i, o in enumerate(ctx.sel.option):
        ...
        if t == OptionType.ABILITY:
            ...
            if cid == CAPE:
                mk = ctx.main_kanga()
                s = 850
                if target is not None and mk is not None and target.serial == mk.serial:
                    s += 20
                elif target is not None and target.id == KANGA:
                    s += 10
                elif target is not None and target.id == CRUSTLE:
                    s += 5
                elif target is not None and target.id == LATIAS:
                    s = 2
